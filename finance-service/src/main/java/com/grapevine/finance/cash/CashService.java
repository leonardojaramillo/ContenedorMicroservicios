...
import com.grapevine.finance.bank.BankAccountRepository;
import com.grapevine.finance.cash.dto.*;
import com.grapevine.finance.client.AuditClient;
...
public class CashService {
    ...
    public CashRegisterResponse open(OpenCashRegisterRequest request) {
        ...
        try {
            auditClient.record(new CreateAuditLogRequest("CAJA_ABIERTA", "Apertura de caja con S/ " + saved.getOpeningAmount(), null));
    ...
    public CashRegisterResponse close(CloseCashRegisterRequest request) {
        ...
        CashRegister saved = cashRegisterRepository.save(cashRegister);
        try {
            auditClient.record(new CreateAuditLogRequest("CAJA_CERRADA", "Cierre de caja con saldo S/ " + saved.getClosingAmount(), null));
        } catch (Exception ignored) {}
    ...
    public CashMovementResponse createMovement(CreateCashMovementRequest request) {
        ...
        try {
            auditClient.record(new CreateAuditLogRequest("MOVIMIENTO_CREADO", "Movimiento de caja registrado por S/ " + saved.getAmount(), null));
        } catch (Exception ignored) {}
    ...
    public CashMovementResponse updateMovementStatus(Long id, MovementStatus status) {
        boolean isAuthorized = SecurityContextHolder.getContext().getAuthentication().getAuthorities().stream()
                .anyMatch(a -> a.getAuthority().equals("ROLE_ADMIN") || a.getAuthority().equals("ROLE_SOFTWARE_ENGINEER"));
        ...
        CashMovement saved = cashMovementRepository.save(movement);
        try {
            auditClient.record(new CreateAuditLogRequest("MOVIMIENTO_" + status, "Movimiento #" + saved.getId() + " " + status, null));
        } catch (Exception ignored) {}
